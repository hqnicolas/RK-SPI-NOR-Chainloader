...
struct __attribute__((packed)) RkImageEntryV2 {
	uint16_t offset;
	uint16_t size;
	uint16_t address;
	uint16_t reserved0;
...
struct __attribute__((packed)) RkHeaderV2 {
	...
	uint32_t boot_flag;
	uint8_t res2[0x68];
	struct RkImageEntryV2 images[4];
};
_Static_assert(sizeof(struct RkImageEntryV2) == 88, "RKNS v2 entry size changed");
_Static_assert(__builtin_offsetof(struct RkImageEntryV2, flag) == 8,
	"RKNS v2 entry padding changed");
_Static_assert(__builtin_offsetof(struct RkHeaderV2, images) == 0x78,
	"RKNS v2 image table offset changed");
_Static_assert(sizeof(struct RkHeaderV2) == 0x1d8, "RKNS v2 header size changed");
